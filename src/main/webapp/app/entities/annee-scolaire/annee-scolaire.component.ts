...
export class AnneeScolaireComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.anneeScolaireService
      ...
      .subscribe(
        (res: HttpResponse<IAnneeScolaire[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInAnneeScolaires(): void {
    this.eventSubscriber = this.eventManager.subscribe('anneeScolaireListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAnneeScolaire[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/annee-scolaire'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
